...
public:
    ...
    QByteArray serializeSubtree(const DocumentNode *node) const override;
...
QByteArray JsonEngine::serializeSubtree(const DocumentNode *node) const
{
    if (!node) return {};
    QJsonValue val = treeToJson(node);
    if (val.isObject()) {
        return QJsonDocument(val.toObject()).toJson(QJsonDocument::Indented);
    } else if (val.isArray()) {
        return QJsonDocument(val.toArray()).toJson(QJsonDocument::Indented);
    } else {
        return valueToString(val).toUtf8();
    }
}
