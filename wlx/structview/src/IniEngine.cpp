...
QByteArray IniEngine::serializeSubtree(const DocumentNode *node) const
{
    if (!node) return {};
    QByteArray result;
    // If it's a section node (General or named)
    if (node->parent && node->parent->name == QStringLiteral("root")) {
        if (node->name != QStringLiteral("General")) {
            result.append('[');
            result.append(node->name.toUtf8());
            result.append("]\n");
        }
